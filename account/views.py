...
from account.forms import NoteForm, UserCreationForm
from account.models import Account, Note
...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f"Account created for {email} You can log in now"
            )
...
def update_note(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request,
                _("An error occurred while attempting to update note."),
            )
    context = {
        "form": form,
        "note": note,
