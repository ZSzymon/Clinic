...
class PatientManager(models.Manager):
    def get_queryset(self, *args, **kwargs):
        return (
            super()
            .get_queryset(*args, **kwargs)
            .filter(type=User.Types.PATIENT)
        )
...
class Appointment(models.Model):
    appointment_start_time = models.DateTimeField(
        _("Appointment start:"), default=timezone.now
    )
    appointment_end_time = models.DateTimeField(
        _("End time"), default=timezone.now
    )
    doctor = models.ForeignKey(
        Doctor,
        related_name=_("DoctorName"),
    ...
    def __str__(self):
        return "{} -> {} on {} ".format(
            self.doctor, self.patient, self.appointment_start_time
        )
